...
#(
  ...
  parameter bit 		 USE_PSE = 1,
  parameter bit 		 DUPLICATE_PMP_CSRS = 0
)
(
  // Clock and Reset
  input  logic                          clk,
  ...
  localparam bit PMP = SECURE || USE_PMP;

  localparam PMP_ADDR_WIDTH = (PMP_GRANULARITY > 0) ? 33 - PMP_GRANULARITY : 32;

  // For pmp snapshot (CL: added)
  localparam PMP_N_SLOT_REGIONS      = 8;

  localparam bit USER = SECURE || USE_PMP;
  ...
  assign csr_err_o = mscratch_rd_error ||
    ...
    priv_lvl_rd_error ||
    mstateen0_rd_error ||
    ( pse_rd_error && (priv_lvl_n == PRIV_LVL_U) ) ||
    mcause_rd_error;
  ...
  begin
    ...
    case (csr_raddr)
      ...
      CSR_PMPADDR56, CSR_PMPADDR57, CSR_PMPADDR58, CSR_PMPADDR59, CSR_PMPADDR60, CSR_PMPADDR61, CSR_PMPADDR62, CSR_PMPADDR63: begin
		...
		CSR_PMP_SNAPSHOT_APPLY: begin
		  if (USE_PSE && PMP) begin
		    csr_rdata_int = PMP_N_SLOT_REGIONS;
		  end else begin
		    illegal_csr_read = 1'b1;
		  end
		end

		CSR_PMP_SNAPSHOT_SAVE: begin
		  if (USE_PSE && PMP) begin
		    csr_rdata_int = PMP_N_SLOT_REGIONS;
		  end else begin
		    illegal_csr_read = 1'b1;
  ...
  begin
    ...
    dcsr_n        = csr_next_value(dcsr_t'{
                                            ...
                                            cause     : dcsr_rdata.cause
                                                // default: 'b0, /* Precautiously changed so it will not cause any diffucult bugs like the struct below */
                                          },
    ...
    if (csr_we_int) begin
      case (csr_waddr)
        ...
        // --- For snapshot unit ---
        CSR_PMP_SNAPSHOT_APPLY: begin // 0x7C0
          if (USE_PSE) begin
            pmp_addr_we = '0; // The write will happen one cycle later and is controlled by write_all_addrs_o
          end
        end

        CSR_PMP_SNAPSHOT_SAVE: begin // 0x7C1
          if (USE_PSE) begin
            // Nothing will be changed in the PMP configs/addrs
            pmp_addr_we = '0; 
          end
        end
  ...
  logic                                       snapshot_write_all_pmp_csrs;
  reg [PMP_ADDR_WIDTH-1:0]                    snapshot_pmp_addrs [PMP_MAX_REGIONS];
  reg [PMP_ADDR_WIDTH-1:0]                    snapshot_pmp_addrs_mem [PMP_MAX_REGIONS];
  reg [7:0]                  snapshot_pmp_configs_mem [PMP_MAX_REGIONS];
  logic [2:0] pse_slot_select;
  reg [2:0] pse_slot_select_reg;
  
/*
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      pse_slot_select_reg         <= 1'b0;
    end else begin
      if (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_APPLY)) 
      begin
        pse_slot_select_reg <= csr_wdata_int[2:0];
      end
    end
  
  end
  
  // For the PSE-CSR comparison, the slot select value must be maintained to read the active PSE-entry 
  logic pse_selected;
  assign pse_selected = (csr_waddr == CSR_PMP_SNAPSHOT_SAVE) || (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_APPLY));
  assign pse_slot_select = pse_selected ? csr_wdata_int[2:0] : pse_slot_select_reg;*/


// Added 04.09.
generate
  if (USE_PSE) begin : g_pse

    // slot select register
    always_ff @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
        pse_slot_select_reg <= '0;
      end else if (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_APPLY)) begin
        pse_slot_select_reg <= csr_wdata_int[2:0];
      end
    end

    // combinational select: current write vs latched
    logic pse_selected;
    assign pse_selected   = (csr_waddr == CSR_PMP_SNAPSHOT_SAVE) ||
                            (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_APPLY));
    assign pse_slot_select = pse_selected ? csr_wdata_int[2:0] : pse_slot_select_reg;

    // The other signals (snapshot_*_mem etc.) will be driven by the PSE block you instantiate elsewhere.

  end else begin : g_no_pse
    // No flops when feature is off; provide deterministic tie-offs.
    assign pse_slot_select = '0;

    // If these get used downstream, tie them off to avoid Xs.
    for (genvar r = 0; r < PMP_MAX_REGIONS; ++r) begin : t
      assign snapshot_pmp_addrs_mem[r]   = '0;
      assign snapshot_pmp_configs_mem[r] = '0;
      assign snapshot_pmp_addrs[r]       = '0;
    end

    assign snapshot_write_all_pmp_csrs = 1'b0;
  end
endgenerate
...
generate
  if (USE_PSE && (PMP_NUM_REGIONS > 0)) begin : csr_pse

      csr_pmp_snapshot_storage 
      #(
        .PMP_ADDR_WIDTH ( PMP_ADDR_WIDTH ),
        .PMP_CONFIG_WIDTH ( 8 ),
        .PMP_N_SLOT_REGIONS ( PMP_N_SLOT_REGIONS )
      )
      csr_pmp_snapshot_storage_0
      (
        .clk(clk),
        .rst_n(rst_n),
        .slot_select        (pse_slot_select),
        .save_i             (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_SAVE)),
        .apply_i            (csr_we_int && (csr_waddr == CSR_PMP_SNAPSHOT_APPLY)),
        .write_all_pmp_csrs_o  (snapshot_write_all_pmp_csrs),
        .pmp_addrs_i        (pmp_addr_q),
        .pmp_configs_i      (pmpncfg_q),
        .pmp_addrs_mem_o    (snapshot_pmp_addrs_mem),
        .pmp_configs_mem_o  (snapshot_pmp_configs_mem),
        .rd_error_o         (pse_rd_error)
      );
 end : csr_pse
  else begin : csr_pse_unused
    // empty - prevents unnamed generate region merging
  end : csr_pse_unused
endgenerate
      ...
      for (genvar i=0; i < PMP_MAX_REGIONS; i++) begin: gen_pmp_csr
        ...
        if (i < PMP_NUM_REGIONS) begin: pmp_region
          ...
          always_comb begin
            ...
            if ( !USE_PMP || (csr_waddr != CSR_PMP_SNAPSHOT_APPLY))
            begin
            ...
            begin
              // This part is for the PMP snapshot add-on
              pmpncfg_n[i] = snapshot_pmp_configs_mem[i];
            end
            end
          end
          ...
          #(
            ...
            .SHADOWCOPY (DUPLICATE_PMP_CSRS), // SECURE), CL: 14.04.25
          ...
          (
            ...
            .wr_en_i        ( pmpncfg_we[i] || snapshot_write_all_pmp_csrs ),
          ...
          always @* 
          begin
            if ( !USE_PSE || (csr_waddr != CSR_PMP_SNAPSHOT_APPLY) )
            begin
              // Keep old data if PMPADDR is locked
              // Bit 0 needs special handling because modification of the read data in TOR and OFF mode could lead to
              // wrong data being used as basis for set/clear operations.
              ...
              // A set operation with mask 0 would then result in PMPADDRn = 0 | 32'hFFFFFFFE, which would clear the LSB of PMPADDRn.
              // Using the value from pmp_addr_q directly avoids this issue.
              pmp_addr_n[i] = pmpaddr_locked[i] ? pmp_addr_q[i] : {csr_wdata_int[31-:(PMP_ADDR_WIDTH-1)], pmp_addr_bit_0_n[i]};
            end else 
            begin
              // This part is for the PMP snapshot add-on
              pmp_addr_n[i] = snapshot_pmp_addrs_mem[i];
            end
          end

if (!USE_PSE) begin : csr_no_pse
          cv32e40s_csr
          #(
            .LIB        (LIB),
         ...
         cv32e40s_csr
          #(
            ...
            .WIDTH      (PMP_ADDR_WIDTH),
            .MASK       (CSR_PMPADDR_MASK[31-:PMP_ADDR_WIDTH]),
            .SHADOWCOPY (0), // SECURE), CL: 14.04.25
            .RESETVALUE (PMP_PMPADDR_RV[i][31-:PMP_ADDR_WIDTH])
          )
          pmp_addr_csr_i
          (
            ...
            .scan_cg_en_i   ( scan_cg_en_i          ),
            .wr_data_i      ( pmp_addr_n[i]         ),
            .wr_en_i        ( pmp_addr_we[i] || snapshot_write_all_pmp_csrs ),
            .rd_data_o      ( pmp_addr_q[i]         ),
            .rd_error_o     ( pmp_addr_rd_error[i]  )
          );
end 
