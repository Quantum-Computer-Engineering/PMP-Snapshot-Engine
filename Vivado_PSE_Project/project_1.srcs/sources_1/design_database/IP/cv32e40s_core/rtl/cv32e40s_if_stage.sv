...
// 	- Added a feature that can be activated by PMP_ENCRYPTION_ENABLED that uses 
//    unused bits in the PMP config registers that are made available outside
//    of the core to optionally activate a memory encryption module.
//  - Reduced MAX_OUTSTANDING to one because the memory side of the SoC 
...
(
  ...
  localparam int unsigned MAX_OUTSTANDING = 1; // CL: Changed from 2 to 1, but with 1 it has not been properly tested so the memory system should be debugged in a way that it is able to process 2 outstanding requests
  ...
  (
    ...
    .core_mpu_err_o       ( mpu_err_o                   ), // Unconnected on purpose
