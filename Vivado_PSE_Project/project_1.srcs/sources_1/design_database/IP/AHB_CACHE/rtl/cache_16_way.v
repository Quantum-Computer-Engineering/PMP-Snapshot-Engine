...
///////////////////////////////////////////////////////
 module	cache_16_way
					...
					(
						...
						input	[ADDRESS_SIZE-1	    :0]	cpu_address_pulsed,
						input	[(WORD_WIDTH*8)-1	:0]	cpu_din_pulsed,
						output	reg [(WORD_WIDTH*8)-1	:0]	cpu_dout,						

						// In the case of the CV32E40S core, the req signal
						// is only set high for one cycle, but this cache expects
						// the signal to be high during the whole transaction.
						// Therefore, the cpu_req_pulsed signal is used and the
						// cpu_req is set to 0.
						input							cpu_req_pulsed_trigger,
						input							cpu_req,

						input							cpu_rw_en_pulsed,
						output		  					cache_hit_o,
						input	[3:0]					cpu_byte_en_pulsed,
...
/////////////////////////////////////////////////////////////////////////////////////
 ...
 // Added for pulsed request (meaning input signals can change during transaction)
 wire 						cpu_req_pulsed;
 wire [ADDRESS_SIZE-1:0] 	cpu_address;
 wire [31:0] 				cpu_din;
 wire 						cpu_rw_en;
...
// Logic to handled pulse req inputs
reg cpu_req_pulsed_reg;
reg cpu_address_hold_reg;
reg cpu_rw_en_reg;
reg [3:0] cpu_byte_en_reg;
reg [(WORD_WIDTH*8)-1	:0] cpu_din_reg;
reg [ADDRESS_SIZE-1:0] cpu_address_reg;

always @(posedge clock, negedge reset)
begin
	if (!reset)
	begin
		cpu_req_pulsed_reg 		<= 0;
		cpu_address_reg 		<= 0;
		cpu_address_hold_reg	<= 0;
		cpu_din_reg 			<= 0;
		cpu_byte_en_reg     	<= 0;
		cpu_rw_en_reg     		<= 0;
	end
	else if (cpu_req_pulsed_trigger)
	begin
		cpu_req_pulsed_reg 		<= 1;
		cpu_address_hold_reg 	<= 1;
		cpu_address_reg	   		<= cpu_address_pulsed;
		cpu_din_reg 			<= cpu_din_pulsed;
		cpu_rw_en_reg     		<= cpu_rw_en_pulsed;
		cpu_byte_en_reg     	<= cpu_byte_en_pulsed;
	end
	else if (cache_hit_o)
	begin
		// As long as there is no hit, the address must be hold,
		// otherwise a different value is sent back to the CPU
		cpu_address_hold_reg <= 0;
		cpu_address_reg 	<= 0;
		cpu_req_pulsed_reg 	<= 0;
		cpu_din_reg 			<= 0;
...
// / Critical path improvment
 assign cpu_req_pulsed 	= cpu_req_pulsed_reg;
 assign cpu_address 	= cpu_address_reg;
 assign cpu_din 		= cpu_din_reg;
 assign cpu_byte_en 	= cpu_byte_en_reg;
 assign cpu_rw_en 	 	= cpu_rw_en_reg;
...
/////////////////////////////////////////////////////////////////////////////////////   
  ...
  begin
		...
		begin
			state <= IDLE;
	 		// cache_hit_o <= 0; // crit path improve
			// cache_hit_o <= 1'b0;
		end
		else
		begin
	 		// cache_hit_o <= (state == HIT); // crit path improve
			state <= next_state;		
			// cache_hit_o <= (state == HIT);
		end
  end

	// crit path improvement
	assign cache_hit_o = (state == HIT);
    ...
    for ( i = 0; i < NUMBER_OF_SETS; i = i + 1 ) begin : sets_gen
		...
		sets
			(
				...
				.req((cpu_req || cpu_req_pulsed) & (set_index==i)), // & (~cache_hit_o)),								
				.wr_back((state == WBACK)),
				
				.cache_cpu_tag(tag),					
				.cache_cpu_dout(cache_cpu_out[i]),
				.cache_cpu_din(cache_cpu_in),
				.cache_cpu_wren(cpu_rw_en),
