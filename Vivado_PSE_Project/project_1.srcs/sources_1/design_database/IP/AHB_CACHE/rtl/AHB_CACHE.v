...
cache
	(
		...
		// "CPU" side
		.cpu_req 		(cache_req & ~(HTRANS == 2)), // in
		.cpu_rw_enable	(APhase_HWRITE), // in
		.cpu_address  	(APhase_HADDR[31:0]), // in
		.cpu_din 		(cache_din), // in
		.cpu_ready   	(cache_rdy), // out
		.cpu_dout 		(HRDATA), // out
		.cpu_byte_en	({byte3,byte2,byte1,byte0}), // in

		// Memory side
        .mem_req		(BRAM_MEM_REQ), // out
		.mem_rw_enable 	(BRAM_MEM_WRITE), // out
		.mem_address	(BRAM_MEM_ADDR), // out
		.mem_dout 		(BRAM_WDATA), //out
		.mem_ready		(BRAM_MEM_VALID), // in
		.mem_din 		(BRAM_RDATA), // in
