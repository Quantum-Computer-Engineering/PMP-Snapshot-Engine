...
//Set Registers for AHB Address State
  ...
  //Reset or write to baud rate register (address xxxxxx04)
  ...
  //UART read select  (address xxxxxx00)
  assign uart_rd = last_HTRANS[1] & ~last_HWRITE & last_HSEL & (last_HADDR[7:0]==8'h00);

  // The read access to 1000_0005 is actually done by the LSU reading 1000_0004 and shifting it by 8
  // That is why HRDATA holds the data as if it was requested for ...04.
  assign HRDATA = (last_HADDR[7:0]==8'h00) ? {24'h0000_00,8'h00}:{16'h0000, status, 8'h00}; // 8'h00 instead of uart_rdata to avoid XX
  assign status = {1'b0, tx_empty, ~tx_full,4'b0000,rx_empty};
